...
{	
	...
	{				
		...
		if ($this->form_validation->run() or $this->current_user)
		{
			...
			{
				$user = $this->ion_auth->get_user_by_username($user->username);
				// print_r($user);die;
				$user->password = '';
				$user->salt = '';
			...
			else
			{
				...
				$this->session->set_flashdata('msg', array('type' => 'success', 'message' => 'You have Logged in Successfully to your account!'));
		...
		if ($_POST)
		{
			...
			$this->session->set_flashdata('msg', array('type' => 'error', 'message' => 'Invalid Login'));
			redirect('');
		}
	...
	public function checkUser() {
		$result = $this->user_m->getUserName($this->input->post('username'));
		if($result){
	...
	public function checkEmail() {
		...
		$result = $this->user_m->getEmail($this->input->post('email'));
		if($result){
	...
	public function logout()
	{
		...
		else
		{
			...
			$this->session->set_flashdata('msg', array('type' => 'success', 'message' => 'You have log out successfuly'));
			redirect('');
		}
	}
	...
	{	
		...
		{
			$this->session->set_flashdata('msg', array('type' => 'info', 'message' => 'User already logged in!'));
		...
		if(isset($_POST['email']) && $_POST['email'] != '' ){
			...
			$this->form_validation->set_rules('email',lang('global:email'),'required|max_length[60]|valid_email');
		}


		if(isset($_POST['mobile']) && $_POST['mobile'] != '' ){

			$this->form_validation->set_rules('mobile','Phone number is required','required');
		}
		...
		{	
			if ($this->form_validation->run())
			{
				...
				if (escape_tags($this->input->post('d0ntf1llth1s1n')) !== ' ')
				{
					...
					$this->session->set_flashdata('msg', array('type' => 'info', 'message' => lang('user:register_error')));
				...
				if(isset($_POST['email']) && $_POST['email'] != '' ){
					...
					$email = escape_tags($this->input->post('mobile'));
				}
				...
				$id = $this->ion_auth->register($username, $password, $email, null, $profile_data, false);
				...
				if(empty($id)){
					...
					if(isset($_POST['email']) && $_POST['email'] != ''){
						$this->db->query("update default_profiles set user_email = '".$mail."' where user_id = ".$id);
					}else{
						$this->db->query("update default_profiles set mobile = '".$mail."' where user_id = ".$id);
					}
				...
				{
					if(isset($_POST['mobile']) && $_POST['mobile'] != ''){
						$pc = $this->db->query("select phonecode from default_country where nicename = '".$this->input->post('country')."'")->result_array();

						$mobile = '+'.$pc[0]['phonecode'].$this->input->post('mobile');
						...
						try {
							...
							$this->db->where("id",$id);
							$this->db->limit(1);
							$this->db->update("default_users",array("activation_code"=>$otp));
                  ...
                  // trigger an event for third party devs
					...
					{	
						...
						try {
							$client->messages->create(
								...
								array(
									'from' => '+14088907359',
									'body' => "Congrats! you are successfully signed up with storeshere"
								)
							);
						} catch (Exception $e) {
					...
					{	
						...
						if(isset($_POST['email']) && $_POST['email'] != ''){
							redirect('signup_success/'.$id);
						}else{
							redirect('signup_mobile/'.$id);
						}
				...
				{
					$this->template->error_string = $errors = $this->ion_auth->errors();
					
					$res['status'] = false;
					$res['message'] = "Email Already Used or Invalid";
					$this->session->set_flashdata('msg', array('type' => 'error', 'message' => 'Email Already Used or Invalid'));
			...
			else
			{
				...
				$this->template->error_string = $errors = $this->form_validation->error_string();
				$res['status'] = false;
				$res['message'] = $errors;
				$this->session->set_flashdata('msg', array('type' => 'error', 'message' => 'The username you selected is already in use'));
				redirect('signup/2');
		...
		try {
			$client->messages->create(
				...
				array(
					'from' => '+14088907359',
					'body' => "Congrats! you are successfully signedup with storeshere"
				)
			);
		} catch (Exception $e) {
	...
	public function ressend_email($value=''){	   	 
		if(!empty($value)){
			$user = $this->ion_auth->get_user_by_id($value);
			if(!empty($user)){
				if($user->active == 0){
					...
					$id = $user->id;
						   		// send Email
					$email = $this->ion_auth->activation_email($value);
						   		// print_r($email);die;
					if ($email > 0){	
						$res['status'] = true;
						$res['message'] = "Successfully Send Message";
						redirect('signup_success/'.$id);
					}else{
						redirect('signup_success/'.$id);
					}
	...
	{	
		...
		if ($this->input->post('email'))
		{
			...
			if ($user_meta)
			{
				...
				{
					//set success message
					//$this->template->success_string = $success = lang('forgot_password_successful');
					$res['status'] = true;
					$res['message'] = "New password set successfully";
				}
				else
				{
			...
			else
			{
				...
				$this->template->error_string = $errors = lang('user:forgot_incorrect');
				$res['status'] = false;
				$res['message'] = "Email address is not registered with us";
			}
			exit(json_encode($res));
		}
        // code is supplied in url so lets try to reset the password

		if ($code &&  $this->user_m->getUserIdByCode($code))
		{
			$data['code'] = $code;
			$this->template->title(lang('user:password_reset_title'))->build('stores/reset_pass',$data);
		}else{
			show_404();
		}
		...
		//echo "404 Page Missing 2!<br>The page you are trying to access doesn't exist.";die;
	...
	public function _check_login($username)
	{
		...
		Events::trigger('login_failed', $username);
	...
	public function contact(){
   	...
   	// error_reporting(E_ALL);
		...
		$g_response = $post['g-recaptcha-response'];
   	  ...
   	  //$response = file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=6LejsJoUAAAAAPgOBHiIDal6Fr7s93kECPrgpVys&response=".$g_response);

   	  //print_r($response); die();   	  
		if ($g_response) {

			$resp = Events::trigger('email', array(
				'name' => $name,
				'sender_ip' => $this->input->ip_address(),
				'sender_agent' => $this->agent->browser().' '.$this->agent->version(),
				'sender_os' => $this->agent->platform(),
			...
			if(json_encode($resp) == '[true]'){
				$res['status'] = true;
				$res['message'] = 'success';
			}else{
				$res['status'] = false;
				$res['message'] = 'Error in connection';
			}
		}else{
			$res['status'] = false;
			$res['message'] = 'Captch varification failed';
		}
		exit(json_encode($res));
	}
}