...
// 			->join('groups', 'groups.id = users.group_id')
    ...
    public function ajax(){
        ...
        $search_column = $_POST['search_column_name'];
        ...
        if($columnName == 'first_name') {
            $columnName = 'dp.first_name';
        }
        elseif($columnName == 'funds/photos') {
            $columnName = 'sfa.send_funds_amount';
        }
        elseif($columnName == 'email/phone') {
            $columnName = 'du.email';
        }
        elseif($columnName == 'group_name') {
            $columnName = 'g.description';
        }
        elseif($columnName == 'inmates') {
            $columnName = 'dia.is_verified';
        }
        elseif($columnName == 'active') {
            $columnName = 'du.active';
        }
        elseif($columnName == 'created_on') {
            $columnName = 'du.created_on';
        }
        else {
            $columnName = 'du.last_login';
        }
        
        // get data from db
        
		$users = $this->user_m->get_ajax_data($columnName, $columnSortOrder, $row, $rowperpage, $search_column, $searchValue);
		
		$total = $this->user_m->all_get_ajax_data($columnName, $columnSortOrder, $search_column, $searchValue);
		
		$totalRecords = count($total);
		
		foreach ($users as $user) {
		    $email_phone = $user->email ? $user->email : $user->phone;
		    $send_funds_amount = $user->send_funds_amount ? $user->send_funds_amount : 0;
		    $send_photos_amount = $user->send_photos_amount ? $user->send_photos_amount : 0;
		    $send_funds_photos = $send_funds_amount.' / '.$send_photos_amount;
		    $data[] = array( 
              "checkbox"=>form_checkbox('action_to[]', $user->id),
              "first_name"=> $user->first_name.' '.$user->last_name,
              "funds/photos"=> anchor('admin/users/sends/' .  $user->id, $send_funds_photos, 'target="_blank" class="modal-large center"'),
              "email/phone"=>$user->user_email != ''  ? $user->user_email : $user->phone,
              "group_name"=>$user->group_name,
              "inmates"=>$user->is_verified == '1' ? anchor('admin/users/user_inmates/' . $user->id, "<b>View Inmates</b>", 'target="_blank" class="modal-large"') : anchor('admin/users/user_inmates/' . $user->id, "View Inmates", 'target="_blank" class="modal-large"'),
              "active"=>$user->active ? lang('global:yes') : lang('global:no'),
              "created_on"=>$user->created_on,
              "last_login"=>$user->last_login,
              "edit/delete"=>anchor('admin/users/edit/' . $user->id, lang('global:edit'), array('class'=>'button edit')).anchor('admin/users/delete/' . $user->id, lang('global:delete'), array('class'=>'confirm button delete')),
            );
		}
	...
	{
	    $sends_photos = $this->profile_m->getSendPhotos($id);
	    $sends_funds = $this->profile_m->getSendFunds($id);
	   // var_dump($sends_funds); die();
	...
	{		
		...
		if(!empty($this->input->post()) && $this->input->post('group_id') == 3 ){			
			array_push($profile_validation, array('field' => 'facility','label' => 'Facility', 'rules' => 'required'));			
		}
		...
		if(isset($profile_data['facility'])){
			if(!empty($profile_data['facility'])){
				$json = explode(',', $profile_data['facility']);
				$profile_data['facility_id'] = json_encode($json);
			}else{
				$profile_data['facility_id'] = $profile_data['facility'];
			}
		}		
		...
		$member->facility_id = ''; // Adding this so i don't get any error
		$this->template
	...
	{		
        ...
        // echo $member->email." : ".$this->input->post('email'); exit();
		// Check to see if we are changing emails
		if ($member->email != "" && $member->email != $this->input->post('email'))
		{
		  //  echo "Inside if";exit;
			$this->validation_rules['email']['rules'] .= '|callback__email_check';
		}
		...
		{
		    if($id != 0)
		        $this->validation_rules['email']['rules'] = 'required|max_length[60]';
		}
        ...
        $profile_validation[5]["rules"] = '';
        
        // var_dump(array_merge($this->validation_rules, $profile_validation));die;
		// Set the validation rules
		$this->form_validation->set_rules(array_merge($this->validation_rules, $profile_validation));
		...
		if ($this->form_validation->run() === true)
		{
			...
			if($this->input->post('facility')){
				// $profile_data['facility_id'] = $this->input->post('facility');
				if(isset($profile_data['facility'])){
					if(!empty($profile_data['facility'])){
						$json = explode(',', $profile_data['facility']);
						$profile_data['facility_id'] = json_encode($json);
...
// 			echo "<pre>".print_r($update_data);	echo "<pre>".print_r($profile_data);exit();
	...
	public function getAllFacilities(){
		$data = $this->user_m->getAllFacilities();
		exit(json_encode($data));
	}
    
    //Code by RS
	/**
	 * List all inmates by user id
	 */
	public function user_inmates($id)
	{	
		
		$inmates = $this->profile_m->getInmatesList($id);

		$this->template
			->set_layout('modal', 'admin')
			->set('inmates', $inmates)
			->build('admin/tables/inmates_list');
	}

	public function edit_inmate($id){
		$res = $this->profile_m->getInmateDetails($id);
		if(isset($res[0]))
			$inmate = $res[0];
		else
			$inmate = [];

		$facility = $this->user_m->getAllFacilities();
		// echo print_r($inmate);exit;
		$this->template
			->set_layout('modal', 'admin')
			->set('inmate', $inmate)
			->set('facility', $facility)
			->build('admin/editinmate');
	}
	...
	public function updateInmate(){
        ...
        if(!empty($post)){
            ...
            $check_0 = isset($_POST['is_verified']) ? 1 : 0;
            $data->is_verified = $check_0;
            $res = $this->profile_m->updateInmate($data, $id);
            if($res){
           		//$this->session->set_flashdata('success', 'Inmate detail updated successfully');
				// redirect('admin/users');
                exit('Inmate detail updated successfully');
            	...
            	// $this->session->set_flashdata('success', 'Some error had occured');
				// redirect('admin/users');
                exit('Some error had occured');
    ...
    public function show_funds_photos($id = 0){
	    if ( ! ($member = $this->ion_auth->get_user($id)))
		{
			$this->session->set_flashdata('error', lang('user:edit_user_not_found_error'));
			redirect('admin/users');
