...
class Order_m extends MY_Model
{
	...
	public function getPhotosDetails($params = array()){
		$query = "SELECT CONCAT(profile.first_name,' ',profile.last_name) as user_name, user.email,profile.mobile,profile.user_email, concat(inmate.inmates_name, ' ', inmate.inmates_middle_name, ' ', inmate.inmates_last_name) inmates_name, inmate.inmates_booking_no, facility.name as facility_name,photo.id,photo.destruction_clause,photo.sent_by,photo.no_of_photos,photo.sent_date,photo.status,photo.invoice,photo.document,photo.text_message,photo.payment_type, payment.amount
...
LEFT JOIN default_payments as payment on photo.payment_id = payment.id";
 // echo $query;die;
	...
	public function getFundDetails($id){
						$query = "SELECT CONCAT(profile.first_name,' ',profile.last_name) as user_name,profile.mobile,profile.user_email,profile.user_id,profile.country, user.email, concat(inmate.inmates_name, ' ', inmate.inmates_middle_name, ' ', inmate.inmates_last_name) inmates_name,inmate.inmates_booking_no, facility.name as facility_name,facility.address,facility.city,facility.county,facility.state,facility.country as fac_country,fund.id,fund.paid_by,fund.paid_to,fund.sent_date,fund.status,fund.invoice,fund.fee,fund.unit,fund.original_amount,fund.processing_fees, payment.amount
...
WHERE fund.id = ".$id;
	...
	public function getPhotoDetails($id){
		$query = "SELECT CONCAT(profile.first_name,' ',profile.last_name) as user_name, user.email,profile.mobile,profile.country, concat(inmate.inmates_name, ' ', inmate.inmates_middle_name, ' ', inmate.inmates_last_name) inmates_name,inmate.inmates_booking_no, facility.name as facility_name,facility.address,facility.city,facility.county,facility.state,facility.country as fac_country,photo.id,photo.sent_by,photo.no_of_photos,photo.sent_date,photo.status,photo.invoice,photo.payment_type, payment.amount
FROM default_send_photos as photo
LEFT JOIN default_users as user on photo.sent_by = user.id
LEFT JOIN default_profiles as profile on photo.sent_by = profile.user_id
LEFT JOIN default_inmates as inmate on photo.sent_to = inmate.inmates_id
...
WHERE photo.id = ".$id;
	...
	public function multiFundStatusUpdate($id, $status){
	    $each_id = explode(",", $id);
	    for($i = 0; $i < count($each_id); $i++) {
	        if($status == 3){
	            $data = array(
                    'status' => $status,
	        ...
	        $this->db->where('id', $each_id[$i]);
	        $this->db->update('default_send_funds', $data);
	    }
	    return $each_id[1];
	}

	public function getFundsData($params = array()){
 		$query = "SELECT user.id user_id, CONCAT(profile.first_name,' ',profile.last_name) as user_name, user.email, concat(inmate.inmates_name, ' ', inmate.inmates_middle_name, ' ', inmate.inmates_last_name) inmates_name, inmate.is_verified, inmate.inmates_booking_no, facility.name as facility_name,fund.id,fund.paid_by,fund.paid_to,fund.sent_date,fund.status,fund.invoice,fund.fee,fund.unit,fund.original_amount,fund.processing_fees,fund.payment_type,fund.delivered_date, payment.amount
            FROM default_send_funds as fund
            LEFT JOIN default_users as user on fund.paid_by = user.id
            LEFT JOIN default_profiles as profile on fund.paid_by = profile.user_id
            LEFT JOIN default_inmates as inmate on fund.paid_to = inmate.inmates_id
...
WHERE photo.id = ".$id;
	...
	public function getUsersByInmate($id){
	    $sql = "select concat(dp.first_name, ' ', dp.last_name) username, du.email, concat(di.inmates_name, ' ', di.inmates_middle_name, ' ', di.inmates_last_name) inmates_name, di.inmates_booking_no
	    from default_inmates di 
	    left join default_users du on du.id=di.user_id
	    left join default_profiles dp on dp.user_id=du.id
	    where di.inmates_booking_no=".$id;
	...
	public function getPhotosSummary($params = array()){
		$query = "SELECT CONCAT(profile.first_name,' ',profile.last_name) as user_name, user.email,profile.mobile,profile.user_email, concat(inmate.inmates_name, ' ', inmate.inmates_middle_name, ' ', inmate.inmates_last_name) inmates_name, facility.name as facility_name,photo.id,photo.destruction_clause,photo.sent_by,photo.no_of_photos,photo.sent_date,photo.status,photo.invoice,photo.document,photo.text_message,photo.payment_type, payment.amount
			FROM default_send_photos as photo
			LEFT JOIN default_users as user on photo.sent_by = user.id
			LEFT JOIN default_profiles as profile on photo.sent_by = profile.user_id
			LEFT JOIN default_inmates as inmate on photo.sent_to = inmate.inmates_id
        ...
        // $query.="ORDER By photo.id, 'DESC' ";
        		$query .= " ORDER BY photo.id DESC limit 5";
	...
	public function getFundsSummary($params = array()){
 		$query = "SELECT CONCAT(profile.first_name,' ',profile.last_name) as user_name, user.email, concat(inmate.inmates_name, ' ', inmate.inmates_middle_name, ' ', inmate.inmates_last_name) inmates_name, facility.name as facility_name,fund.id,fund.paid_by,fund.paid_to,fund.sent_date,fund.status,fund.invoice,fund.fee,fund.unit,fund.original_amount,fund.processing_fees,fund.payment_type,fund.delivered_date, payment.amount
		FROM default_send_funds as fund
		LEFT JOIN default_users as user on fund.paid_by = user.id
		LEFT JOIN default_profiles as profile on fund.paid_by = profile.user_id
		LEFT JOIN default_inmates as inmate on fund.paid_to = inmate.inmates_id
		...
		$query .= " ORDER BY fund.id DESC limit 5";		
		$result = $this->db->query($query);
